    ...
    <>
      <motion.nav
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
            ? 'bg-white/[0.92] supports-[not(backdrop-filter:blur(0px))]:bg-white backdrop-blur-xl border-b border-[rgba(0,0,0,0.08)]'
            : 'bg-transparent'
        }`}
      >
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            id="mobile-menu"
            className="fixed top-[68px] left-0 right-0 z-40 bg-white border-b border-[rgba(0,0,0,0.08)] shadow-lg"
          >
            <div className="max-w-7xl mx-auto px-6 pt-4 pb-6 flex flex-col gap-1">
              {NAV_LINKS.map((link) => (
                <a
                  ...
                  className="font-sans text-text-secondary hover:text-text-primary text-base py-3 border-b border-[rgba(0,0,0,0.08)] last:border-0 transition-colors"
                >
                  {link.label}
                </a>
              ))}
